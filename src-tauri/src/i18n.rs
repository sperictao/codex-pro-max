...
fn zh_cn(key: &str) -> Option<&'static str> {
    Some(match key {
        ...
        "Loopback proxy is running on 127.0.0.1:3898" => "本地回环反代已运行在 127.0.0.1:3898",
        "Cannot write ~/.dsh/loopback-proxy.js; check disk permissions" => "无法写入 ~/.dsh/loopback-proxy.js，请检查磁盘权限",
        "Install Node.js 18+ first" => "请先安装 Node.js 18+",
        "Starting loopback proxy…" => "正在启动本地回环反代…",
        "Port 3898 may be occupied; stop the process using it and retry" => "端口 3898 可能被占用，请先停止占用该端口的进程后重试",
        "Loopback proxy did not start within 10s" => "本地回环反代 10 秒内未能启动",
        "Check the log at ~/.dsh/loopback-proxy.log" => "请查看 ~/.dsh/loopback-proxy.log",
        ...
        "WebSocket handshake failed ({url}/api/events.host); the loopback proxy may be an outdated version without WebSocket upgrade forwarding — rerun the one-click setup to refresh it" => "WebSocket 握手失败（{url}/api/events.host）；回环反代可能是缺少 WebSocket 转发的旧版本——请重跑一键安装以刷新脚本",
        "Restarting dsh web with remote-access flags…" => "正在以远程访问参数重启 dsh Web…",
        "dsh web is running on 127.0.0.1:3899 (restarted with SSH_CONNECTION + --trusted-host)" => "dsh Web 已运行在 127.0.0.1:3899（已带 SSH_CONNECTION + --trusted-host 重启）",
        ...
        "Port 3898 is occupied by another process" => "端口 3898 被其他进程占用",
        "Stop the process listening on 127.0.0.1:3898, then retry" => "请先停止监听 127.0.0.1:3898 的进程，然后重试",
        "dsh web failed to restart" => "dsh Web 重启失败",
        "Restarting loopback proxy…" => "正在重启本地回环反代…",
        "Loopback proxy is running but does not forward WebSocket upgrades" => "回环反代在运行但未转发 WebSocket upgrade",
        "Check the log at ~/.dsh/loopback-proxy.log; the proxy must forward WebSocket upgrades for remote chat to work" => "请查看 ~/.dsh/loopback-proxy.log；远程对话需要反代转发 WebSocket upgrade",
        "Privileged API check failed (POST /api/settings.describe via the loopback proxy); the proxy must strip Origin and rewrite Host to 127.0.0.1" => "敏感 API 校验失败（经回环反代 POST /api/settings.describe）；反代必须删除 Origin 并把 Host 改写为 127.0.0.1",
