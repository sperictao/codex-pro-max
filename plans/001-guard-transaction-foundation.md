...
### Step 4 实施结果（2026-08-10）

- 已在 `engine.rs` 增加无 I/O 的 `plan_file_write`：同一物理文件只解析一次，按规范化路径、
  参数 ID 和模式稳定排序合并成员，候选字节再次校验，并记录原/候选 SHA-256 与内存态
  post-check 描述。
- 已将 `guard_apply`、锁定时恢复和轮询恢复从旧的直写 `engine::apply` 迁移到
  `execute_single_plan`；该入口先生成单成员计划，再调用明确标注为过渡 sink 的
  `legacy_write_with_backup`，因此当前仍保留原有副作用，但没有绕过计划器的业务入口。
- `check_many` 被视图和轮询按物理文件复用，读取与格式解析各做一次；TOML 路径、Markdown
  区块和整文件覆盖比对统一走同一套路径/格式语义，整文件覆盖按候选字节比较。
- 新增计划器红测/回归测试：TOML 多成员顺序不变、Markdown 非托管内容保留、语义/类型/路径
  错误 fail-closed、候选复校验、缺失文件边界、SHA-256 与规划阶段不写磁盘。
...
实现层已覆盖格式管线、所有权预检、单文件/联合参与者事务、协调器、恢复阻断、durable
backup、journal 安全边界和 IPC 合同；以下清单只统计仍需最终证据的硬门，不把“已编译”当作
崩溃恢复证明。
